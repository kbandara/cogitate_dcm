...
P_Names_struct = load('participants.mat');  
...
for si = 1:length(p_names) 
    ...
    filename = fullfile(filepath, ['ronly_meg_bc_lf_ra_r_ms_re_faces_a_d_spmeeg_sub-' p_names{si} '_ses-1_task-dur_run-01_meg.mat']); 
    ...
    fiducials = readstruct(anat_fid); 
    lpa = fiducials.AnatomicalLandmarkCoordinates.LPA; 
    nas = fiducials.AnatomicalLandmarkCoordinates.NAS;    
    rpa = fiducials.AnatomicalLandmarkCoordinates.RPA;
    ...
    matlabbatch{1}.spm.meeg.source.headmodel.meshing.meshes.mri = {anat_mri}; %create mesh using sMRI
...
for onset = 1:length(p_names) 
    ...
    filename = fullfile(filepath, ['ronly_meg_bc_lf_ra_r_ms_re_faces_a_d_spmeeg_sub-' p_names{onset} '_ses-1_task-dur_run-01_meg.mat']); 
...
for move = 1:length(p_names)
    ...
    for i = 1:8 %for each condition/image generated above 
    
    og_file = fullfile(input_dir, ['ronly_meg_bc_lf_ra_r_ms_re_faces_a_d_spmeeg_sub-' p_names{move} '_ses-1_task-dur_run-01_meg_1_t0_600_f0_256_' num2str(i) '.nii']);
    target_file = fullfile(image_dir, ['ronly_meg_bc_lf_ra_r_ms_re_faces_a_d_spmeeg_sub-' p_names{move} '_ses-1_task-dur_run-01_meg_1_t0_600_f0_256_' num2str(i) '.nii']); 
...
meg_prefix = 'ronly_meg_bc_lf_ra_r_ms_re_faces_a_d_spmeeg_sub-';
meg_suffix = '_ses-1_task-dur_run-01_meg.mat';
nii_file_prefix = 'ronly_meg_bc_lf_ra_r_ms_re_faces_a_d_spmeeg_sub-';
...
for c = 1:length(p_names)
    ...
    % If the directory exists from a previous run, delete it completely to prevent overwrite warning popup (this popup can crash the script)
    if exist(GLM_dir, 'dir')
        rmdir(GLM_dir, 's'); % The 's' flag removes the directory and its contents
    end
    ...
    is_face       = contains(nii_files, face);
    is_other      = contains(nii_files, other);
...
outfile = fullfile(dcm_folder, 'tcon_indiv_mni_coords.m');
...
for c = 1:length(p_names)
    ...
    for i = 1:length(roi_labels)
        ...
        matlabbatch{1}.spm.stats.con.consess{1}.tcon.name = 'Main effect of face_other';
...
for p = 1:length(p_names)
    ...
    for r = 1:length(roi_labels)
        ...
        % try to read the numeric data from the CSV, skipping the 2 header lines.
        try
...
peaks_file = fullfile(dcm_folder, 'tcon_indiv_peak_coords.mat');

save(peaks_file, 'indiv_mni_coords');

%%                    --- Get Peak MNI Coords ---

% Initialize the final cell array to store all coordinates for all
% participants
indiv_mni_coords = cell(length(p_names), 2);
...
for p = 1:length(p_names)
    ...
    % Loop through the ROIs in the specified order
    for r = 1:length(roi_labels)
...
% CA108 had no value for V1s so we will find the group mean value for
% this participant to use as their V1 coordinates
...
output_dir    = fullfile(image_dir, 'GROUP_AVERAGE_V1');
mask_file    = '/data/gpfs/projects/punim2118/Cogitate_DCM/roi_masks/jubrain_masks/R_V1.nii'; % change mask file here!!!

if exist(output_dir, 'dir')
    delete(fullfile(output_dir, '*.*')); % Deletes the old SPM.mat
else
    mkdir(output_dir);
end
FR = {}; 
