...
% Define the end points for each time window (common period: 0-1000ms)
...
for t = 1:length(time_window_increments)
    ...
    % Load short trial GCM
    gcm_file = fullfile(output_dir, ['GCM_' num2str(current_time_window_end) 'ms.mat']);
...
for t = 1:length(time_window_increments)
    ...
    gcm_filename = fullfile(output_dir, ['GCM_' num2str(current_time) 'ms.mat']);
...
for t = 1:length(post_offset_increments)
    ...
    for i = 1:length(GCM) %force load the DCMs (instead of file paths)
        if ischar(GCM{i}) || isstring(GCM{i})
            tmp = load(GCM{i});
            GCM{i} = tmp.DCM;
        end
    end
